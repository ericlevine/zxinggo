...
// codewordTable is the codeword table from the Annex A of ISO/IEC 15438:2001(E).
var codewordTable = [3][929]int{
...
// PDF417Encoder is the top-level struct for the logic part of the PDF417
// implementation.
...
// NewPDF417Encoder creates a new PDF417Encoder with default settings.
...
func (p *PDF417Encoder) SetDimensions(maxCols, minCols, maxRows, minRows int) {
	p.maxCols = maxCols
	p.minCols = minCols
	p.maxRows = maxRows
	p.minRows = minRows
...
func (p *PDF417Encoder) SetCompaction(compaction Compaction) {
	p.compaction = compaction
}

// SetCompact enables or disables compact mode.
func (p *PDF417Encoder) SetCompact(compact bool) {
	p.compact = compact
}

// BarcodeMatrix returns the barcode matrix.
func (p *PDF417Encoder) BarcodeMatrix() *BarcodeMatrix {
	return p.barcodeMatrix
}

// GenerateBarcodeLogic generates the barcode logic for the given message
// and error correction level.
func (p *PDF417Encoder) GenerateBarcodeLogic(msg string, errorCorrectionLevel int) error {
	// 1. step: High-level encoding
	errorCorrectionCodeWords, err := GetErrorCorrectionCodewordCount(errorCorrectionLevel)
	if err != nil {
		return err
	}
	highLevel, err := EncodeHighLevel(msg, p.compaction)
	if err != nil {
		return err
	}
	sourceCodeWords := len([]rune(highLevel))

	dimension, err := determineDimensions(p.minCols, p.maxCols, p.minRows, p.maxRows,
	...
	p.encodeLowLevel(dataCodewords+ec, cols, rows, errorCorrectionLevel, p.barcodeMatrix)

	return nil
}

// calculateNumberOfRows calculates the necessary number of rows as described
// in annex Q of ISO/IEC 15438:2001(E).
...
// encodeLowLevel performs the low-level encoding of the barcode.
func (p *PDF417Encoder) encodeLowLevel(fullCodewords string, c, r, errorCorrectionLevel int, logic *BarcodeMatrix) {
	codewordRunes := []rune(fullCodewords)
	idx := 0
	for y := 0; y < r; y++ {
		cluster := y % 3
		logic.StartRow()
		encodeChar(startPattern, 17, logic.CurrentRow())
		...
		pattern := codewordTable[cluster][left]
		encodeChar(pattern, 17, logic.CurrentRow())

		for x := 0; x < c; x++ {
			pattern = codewordTable[cluster][int(codewordRunes[idx])]
			encodeChar(pattern, 17, logic.CurrentRow())
			idx++
		}

		if p.compact {
			encodeChar(stopPattern, 1, logic.CurrentRow()) // encodes stop line for compact pdf417
		} else {
			pattern = codewordTable[cluster][right]
			encodeChar(pattern, 17, logic.CurrentRow())
			encodeChar(stopPattern, 18, logic.CurrentRow())
		}
	}
}

// determineDimensions determines optimal number of columns and rows for the
// specified number of codewords.
func determineDimensions(minCols, maxCols, minRows, maxRows, sourceCodeWords, errorCorrectionCodeWords int) ([]int, error) {
	ratio := float64(0)
	var dimension []int
	currentCol := minCols
	...
	if dimension == nil {
		return nil, errors.New("unable to fit message in columns")
	}
