...
// ecCoefficients contains the tables of coefficients for calculating error
// correction words (see annex F, ISO/IEC 15438:2001(E)).
var ecCoefficients = [][]int{
	{27, 917},
	{522, 568, 723, 809},
...
// GetErrorCorrectionCodewordCount determines the number of error correction
// codewords for a specified error correction level.
func GetErrorCorrectionCodewordCount(level int) (int, error) {
	if level < 0 || level > 8 {
		return 0, fmt.Errorf("error correction level must be between 0 and 8, got %d", level)
	}
	return 1 << (level + 1), nil
}

// GetRecommendedMinimumErrorCorrectionLevel returns the recommended minimum
// error correction level as described in annex E of ISO/IEC 15438:2001(E).
func GetRecommendedMinimumErrorCorrectionLevel(n int) (int, error) {
	...
	return 0, errors.New("no recommendation possible")
}

// GenerateErrorCorrection generates the error correction codewords according
// to 4.10 in ISO/IEC 15438:2001(E).
func GenerateErrorCorrection(dataCodewords string, level int) (string, error) {
	k, err := GetErrorCorrectionCodewordCount(level)
	if err != nil {
		return "", err
	}
	...
	for i := 0; i < sld; i++ {
		...
		t2 := (t1 * ecCoefficients[level][0]) % 929
		t3 := 929 - t2
		e[0] = t3 % 929
	}
